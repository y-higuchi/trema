...
describe Port do
  ...
  it "should raise error if empty instance is created" do
    lambda {
      port = Port.new
    }.should raise_error()
  end
  ...
  it "should serialize to human readable form by to_s" do
    port = Port[ {:dpid => 0x1234, :portno => 42 } ]
    port.to_s.should == "Port 0x1234:42 - []"
  end
...
describe Link do
  ...
  it "should serialize to human readable form by to_s" do
    link = Link[ {:from_dpid => 0x1234, :from_portno => 42, :to_dpid => 0x5678, :to_portno => 72 } ]
    link.to_s.should == "Link (0x1234:42)->(0x5678:72) - []"
  end

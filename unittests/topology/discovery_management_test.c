...
test_switch_status_event() {
  ...
  UNUSED( helper_sw_message_received_end );
...
static void
helper_sw_message_received_over_ip_end( uint16_t tag, void *data, size_t len ) {
  check_expected( tag );
  UNUSED( data );
  UNUSED( len );
...
static void
test_switch_status_event_over_ip() {
  setup();
  discovery_management_options options;
  options.always_enabled = false;
  ...
  UNUSED( helper_sw_message_received_end );
  const char* SRC_SW_MSNGER_NAME = "switch.0x1234";
  assert_true( add_message_received_callback( SRC_SW_MSNGER_NAME, helper_sw_message_received_over_ip_end ) );

  expect_value( helper_sw_message_received_over_ip_end, tag, MESSENGER_OPENFLOW_MESSAGE );
  expect_value( helper_sw_message_received_over_ip_end, datapath_id, 0x1234 );
  expect_value( helper_sw_message_received_over_ip_end, nw_src, 0x01234567 );
  expect_value( helper_sw_message_received_over_ip_end, nw_dst, 0x89ABCDEF );
  ...
  assert_true( delete_message_received_callback( SRC_SW_MSNGER_NAME, helper_sw_message_received_over_ip_end ) );
...
main() {
  const UnitTest tests[] = {
      ...
      // TODO enable discovery when sw_entry present then flow del
      // TODO disable discovery when sw_entry present then flow del

      unit_test( test_switch_status_event ),
      unit_test( test_switch_status_event_over_ip ),
